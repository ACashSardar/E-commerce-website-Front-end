            ...
            <div className='row'>
                    ...
                    cartItems.map((cartItem,index)=>
                        <div className="shadow bg-light rounded-1 d-flex p-2 mb-2">
                            ...
                            <div className='p-2 px-4' style={{width:"100%"}}>
                                <b className='fs-4'>₹ {cartItem.quantity*cartItem.product.price}</b>
                                <h4>{cartItem.product.productName}</h4>
                                <p>{cartItem.product.productDesc}</p>
                                <div className="d-flex justify-content-between">
                                    <span className='me-2'>
                                        <button className='btn btn-sm btn-dark border fw-bold' onClick={()=>handleUpdateCartItem(cartItem.cartItemId, -1, userInfo)}>-</button>
                                        <span className='fw-bold rounded-1 p-2 px-2'>{cartItem.quantity}</span>
                                        <button className='btn btn-sm btn-dark border fw-bold' onClick={()=>handleUpdateCartItem(cartItem.cartItemId, +1, userInfo)}>+</button>
                                    </span>
                                    <button className='custom-outline-btn' onClick={()=>{handleDeletingCartItem(cartItem.cartItemId, userInfo)}}>Remove</button>
                                </div>
                            </div>
                        </div>
                    )
                ...
                <div className="col-md-4 col-sm-12 shadow bg-light rounded-1 p-4 mt-2 mb-3">
                    ...
                    <div className='fs-6 d-flex justify-content-between'>
                        <button className='custom-outline-btn me-1' onClick={()=>{
                            clearCart(cartItems);
                            toast("Your cart has been cleared.")
                            loadCartitems();
                        }} style={{width:"99%"}}>Clear cart</button>
                        <Link to="/order" style={{width: "100%"}} className='text-decoration-none bg-info' ><button style={{width: "100%"}} className='custom-btn' onClick={()=>handleAddOrder(userInfo)}>Check out</button></Link>
                    </div>
                </div>
            </div>
            :
