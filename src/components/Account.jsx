...
const Account = ({currentUser,loadCurrentUser}) => {
  ...
  const handleEditAccount=(name,address,phoneNo)=>{
    ...
    updateUser(formData,currentUser.userId,userInfo).then((res)=>{
      loadCurrentUser()
      document.getElementById("profile-pic").src=BASE_URL+"/api/v1/users/image/"+currentUser.profilePic
      ...
      <div className='bg-canvas p-5'>
        <ToastContainer />
          <div className=' d-flex justify-content-center'>
              <div className='bg-light rounded px-5'>
                <img id="profile-pic" src={BASE_URL+"/api/v1/users/image/"+currentUser.profilePic} alt="" style={{height: "14rem", width:"14rem"}} />
              </div>
          </div>
          <div className=' d-flex justify-content-center'>
          {
            editing===false?
              <div className="p-4 rounded shadow bg-light" style={{width:"20rem"}}>
                {
                ...
                {
                  currentUser.userId!==""?
                  <button  className='custom-btn' onClick={()=>setEditing(true)} style={{width:"100%"}}>Edit Account</button>
                  :<></>                
                }
                
              </div>
              :
              <div className="p-4 pt-2 rounded shadow bg-light" style={{width:"20rem"}}>
                <input type="file" accept="image/*" className='form-control rounded-1 my-1' style={{width:"100%"}} onChange={(e)=>loadImage(e)} required/>
                ...
                {
                  currentUser.userId!==""?
                  <button  className='custom-btn' onClick={()=>handleEditAccount(name,address,phoneNo)} style={{width:"100%"}}>Update Account</button>
                  :<></>                
                }
              </div>
          }
